...
export async function GET(req: NextRequest) {
  try {
    ...
    if (cronSecret && authHeader !== `Bearer ${cronSecret}`) {
      console.warn('⚠️ Unauthorized cron request attempted')
    ...
    console.log('🕐 Cron job triggered via GET request at:', new Date().toISOString())
    ...
    console.error('❌ Error in cron notification process:', error)
    return NextResponse.json(
      {
...
// POST handler for manual triggers (keep existing functionality)
export async function POST(req: NextRequest) {
  try {
    console.log('🔄 Manual notification trigger via POST request at:', new Date().toISOString())
